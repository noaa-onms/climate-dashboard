---
title: "`r params$sanctuary`"
output: 
  flexdashboard::flex_dashboard:
    vertical_layout: scroll
    self_contained: false
    lib_dir: "docs/libs"
    includes:
      in_header: "./_navbar.html"
    css: "_style.css"
params:
  # nms: "GRNMS"
  # sanctuary: "Gray's Reef"
  # nms: "MBNMS-david"
  # sanctuary: "Monterey Bay - Davidson Seamount"
  # nms: "MNMS"
  # sanctuary: "Monitor"
  # nms: "MBNMS-david"
  # sanctuary: "Monterey Bay - Davidson Seamount"
  # nms: "FGBNMS"
  # sanctuary: "Flower Garden Banks"
  nms: "CBNMS"
  sanctuary: "Cordell Bank"
execute:
  cache: false
editor_options: 
  chunk_output_type: console
---

```{r setup}
# params <- list(
#   nms       = "CBNMS",
#   sanctuary = "Cordell Bank")

librarian::shelf(
  dplyr, glue, here, mapview, readr, sf, stringr, tidyr,
  marinebon/extractr)
# devtools::load_all("~/Github/marinebon/extractr")
source(here("scripts/functions.R"))
options(readr.show_col_types = F)

sanctuaries <- readRDS(here("data/sanctuaries.rds"))

# TODO: change "datasets" to "variables"
datasets <- read_csv(here("data/datasets.csv"))
datasets_ed <- datasets |> 
  filter(type == "erddap")
datasets_nc <- datasets |> 
  filter(type == "nc")
# TODO: add y_label, ∆ height CRW_SST

write_metadata <- function(
    sanctuary_nms, dataset_var, dataset_url,
    prefix = "", suffix = ""){

  cat(glue('
  <details>
  <summary><span style="color: #737373; font-size: 12px">Metadata</span></summary>
  {prefix}
  The ERDDAP dataset for [{dataset_var}]({dataset_url}) was extracted using [get_data.R](https://github.com/noaa-onms/climate-dashboard/blob/main/scripts/get_data.R) for the {params$sanctuary} polygon and output to [{params$nms}.csv](https://raw.githubusercontent.com/noaa-onms/climate-dashboard/main/data/{dataset_var}/{params$nms}.csv), which gets visualized below using [`extractr::plot_ts()`](https://marinebon.github.io/extractr/reference/plot_ts.html).
  {suffix}
  </details> 
  '))
}
```

# Map {.sidebar data-width=200}

```{r}
# params <- list(
#   nms = "GRNMS",
#   sanctuary = "Gray's Reef")

sanctuaries |>
  filter(sanctuary == params$sanctuary) |>
  map_sanctuary()
```

# Climatological

### Sea Surface Temperature {data-height=300}

```{r, results='asis'}
ds_var <- "sst"
ds <- filter(datasets_ed, var == !!ds_var)
write_metadata(params$nms, ds_var, ds$url)
```
    
```{r}
ts_csv <- here(glue("data/{ds_var}/{params$nms}.csv"))
plot_ts(ts_csv, fld_sd = "sd", fld_date = "date", color = ds$plot_color, label = ds$plot_label)
```

### Chlorophyll {data-height=300}
    
```{r, results='asis'}
ds_var <- "chlorophyll"
ds <- filter(datasets_ed, var == !!ds_var) |> 
  slice(1)
# TODO: clarify that "chlorophyll" is actually two datasets: 
# - chl:      https://coastwatch.pfeg.noaa.gov/erddap/griddap/erdMH1chlamday.html
# - chl, nrt: https://coastwatch.pfeg.noaa.gov/erddap/griddap/erdMH1chlamday_R2022NRT.html

write_metadata(params$nms, ds_var, ds$url)
```

```{r}
ts_csv <- here(glue("data/{ds_var}/{params$nms}.csv"))
plot_ts(ts_csv, fld_sd = "sd", fld_date = "date", color = ds$plot_color, label = ds$plot_label)
```

<!-- ### Coral Reef Watch {data-height=300} -->

```{r, results='asis'}
# ds_var <- "CRW_SST"
# ds <- filter(datasets_ed, var == !!ds_var)
# write_metadata(params$nms, ds_var, ds$url)
```

```{r}
# ts_csv <- here(glue("data/{ds_var}/{params$nms}.csv"))
# # TODO: resolve issues with Monitor being too small to have data
# has_data <- sum(!is.na(read_csv(ts_csv, show_col_types=F) |> pull(mean))) > 0
```

```{r}
# eval: !expr has_data

# plot_ts(ts_csv, fld_sd = "sd", color = ds$plot_color, label = ds$plot_label)
```

# Projections

```{r, results='asis'}
source(here("scripts/functions.R"))

clim_csv <- glue(here("data/ucar_projections_by_sanctuary.csv"))

var <- "SST"
has_var <- read_csv(clim_csv) |> filter(nms == params$nms, var == !!var) |> nrow() > 0

if (has_var)
  nc_meta(var)
```

```{r}
#| eval: !expr has_var
nc_plot(var, nms = params$nms)
```

```{r, results='asis'}
var <- "Chl"
has_var <- read_csv(clim_csv) |> filter(nms == params$nms, var == !!var) |> nrow() > 0

if (has_var)
  nc_meta(var)
```

```{r}
#| eval: !expr has_var
nc_plot(var, nms = params$nms)
```

```{r, results='asis'}
var <- "NPP"
has_var <- read_csv(clim_csv) |> filter(nms == params$nms, var == !!var) |> nrow() > 0

if (has_var)
  nc_meta(var)
```

```{r}
#| eval: !expr has_var
nc_plot(var, nms = params$nms)
```

```{r, results='asis'}
var <- "PH"
has_var <- read_csv(clim_csv) |> filter(nms == params$nms, var == !!var) |> nrow() > 0

if (has_var)
  nc_meta(var)
```

```{r}
#| eval: !expr has_var
nc_plot(var, nms = params$nms)
```


```{r, results='asis'}
var <- "SSS"
has_var <- read_csv(clim_csv) |> filter(nms == params$nms, var == !!var) |> nrow() > 0

if (has_var)
  nc_meta(var)
```

```{r}
#| eval: !expr has_var
nc_plot(var, nms = params$nms)
```


```{r, results='asis'}
var <- "HMXL_DR"
has_var <- read_csv(clim_csv) |> filter(nms == params$nms, var == !!var) |> nrow() > 0

if (has_var)
  nc_meta(var)
```

```{r}
#| eval: !expr has_var
nc_plot(var, nms = params$nms)
```

<!--
# Physical-chemical

### Chart P.1

```{r}
```

### Chart P.2

```{r}
```


# Biological

### Chart B.1

```{r}
```

### Chart B.2

```{r}
```

# Human Dimensions

### Chart H.1

```{r}
```

### Chart H.2

```{r}
```

-->
