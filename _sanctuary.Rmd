...
librarian::shelf(
  ...
  marinebon/extractr, lubridate, ggplot2, plotly)
...
SST.lastyear = SST %>% filter(year(date)==max(year(date)))

## make the plot
# pp = ggplot(SST.clim, aes(yDay, SST.mean))
# TODO: add back pp = pp + below
pp = geom_line() + geom_smooth(span=0.25, se=FALSE, colour="steelblue") +  
